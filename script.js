...
//   let selectedShowId = idSelector()

//   getAllEpisodesByFetch(`https://api.tvmaze.com/shows/${selectedShowId}/episodes`).then(episodes => {
...
//     let filteredShow = shows.filter(show => show.name == e.target.value)
//     return filteredShow[0].id
...
// //     getAllEpisodesByFetch(filteredShow[0].url)
// //   })
// // }
  ...
  shows.forEach((show) => {
    ...
    selectShowEl.addEventListener('change', (e) => {
      ...
      filteredEpisodes = shows.filter((show) => {
        // console.log('filtre', show.name, e.target.value);
...
function createCard(episode) {
  ...
  let card = `<div class="col-md-5th-1 col-sm-4">
            <div class="episodeTitle">
              <h4>${episodeName} S${seasonNumber.padStart(
    2,
    '0'
  )}E${episodeNumber.padStart(2, '0')}</h4>
            </div>
            <div class="picture">
            <img src= ${imageElement} alt="episode image">
            </div>
...
function episodeList(episodes) {
  episodes.forEach((episode) => {
    ...
    let seasonNumbers = episode.season.toString();
    let episodeNumbers = episode.number.toString();
    let uniqueCode = (optionEpisodesElement.innerHTML = `S${seasonNumbers.padStart(
      2,
      '0'
    )}E${episodeNumbers.padStart(2, '0')} ${episodeNames}`);
    // creates a unique code for each episode
    episode.uniqueCodeNumber = uniqueCode;
  });
}
